...
void loop(void) {
    ...
    if (should_animate) {
        ...
        if (message_to_rcv.flags.test(0)) {
            p10.draw_pattern_static(reconstructedGrid, 4, 0);
        } else if (message_to_rcv.flags.test(1)) {
            // Serial.println(message_to_rcv.bChar);
            anim_drawMarquee(message_to_rcv.bChar, static_cast<byte>(std::strlen(message_to_rcv.bChar)));
        }
    }
