...
unsigned int cpAdcLimits[6] = {3280, 2390, 1510, 570, 0, 1780}; //%50
...
const int pwmDutyCycle = 50; // ( 50 =22kw ) ( 25=11kw ) ( 17=7kw )
...
void getCpVoltage()
{
    ...
    unsigned int refVoltage = 0;

    refVoltage = analogRead(32);
    ...
    analogValue = analogValue * 3100 / refVoltage;
}
...
void loop_haberlesme()
{
    ...
    if (mainCounter < millis())
    {
        ...
        if (deviceState == 1)
        { // Cihaz bekleme modunda ise (araba takili degil)
        ...
        if (deviceState == 2)
        { // Cihaz arac baglandi ama pwm baslamadi modunda ise
            ...
            case 2: // Arac Baglandi ama ana islemciden PWM baslatma gelmedi ise, beklemede
                ...
                if (digitalRead(COMM_BT3) == HIGH)
                { // ana islemciden pwm baslat emri geldi mi?
                    timerAlarmEnable(freqGenTimer);
                    pwmState = 1;
                }
                deviceStateCode = 22;
        ...
        if (deviceState == 3)
        { // Cihaz arac baglandi, Pwm verildi, aracın sarj almasi bekleniyor ise
            ...
            case 3: // Arac baglandi, pwm verildi, arac sarja baslamadi ise
                if (digitalRead(COMM_BT3) == LOW)
                { // ana islemci tarafindan pwm sonlandirildi ise
                    timerAlarmDisable(freqGenTimer);
                    pwmState = 0;
                    digitalWrite(CP_SIGNAL_IN, HIGH);
                }
        ...
        if (deviceState == 4)
        { // Cihaz arac baglandi ve sarj basladi modunda ise
        ...
        if (deviceState == 5)
        { // Cihaz arac baglandi ve sarj basladi ve havalandirma modunda ise
        }

        if (deviceState == 0)
        { // Cihaz ariza modunda ise
