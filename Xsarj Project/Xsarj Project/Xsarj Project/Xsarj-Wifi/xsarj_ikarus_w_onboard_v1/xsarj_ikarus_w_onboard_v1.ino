...
#define HOST_OF_OTA "http://panel.xsarj.com:8081/web/content/3268?download=true"
HTTPClient client;
// Global variables
...
float alarm_time = 0;
unsigned long setting_1_value = 0;
unsigned long setting_2_value = 0;
...
//#define TX (1)
//#elif defined(ESP32)
#define D5 (100) // esp32 nin rx pini
#define D6 (100) // esp32 nin tx pini
#define D7 (100)
#define D8 (100)
#define TX (1)
//#endif
...
int sensor_2 = 100;
int sensor_3 = 100;
int sensor_4 = 23; //bit1
int sensor_5 = 19; //bit2
int sensor_6 = 18; //bit3
int sensor_7 = 35;
int sensor_8 = 100;
int sensor_9 = 33;
int sensor_10 = 100;
...
int idle_request_counter = 0;
unsigned long previousMillis_idle_request_timer = 0;
const long interval_idle_request_timer = 1000;
...
void setup() {
  ...
  device_Status_Setup();
  Serial.println("---------- Device Status Setup Finish ----------------");
...
void Task1code( void * pvParameters ) {
  ...
  for (;;) {
    ...
    if (currentMillis - previousMillis_idle_request_timer >= interval_idle_request_timer) {
      previousMillis_idle_request_timer = currentMillis;
      idle_request_counter++;
    }
    if (nfc.tagPresent())
    {
      NfcTag tag = nfc.read();
    ...
    if (WiFi.status() == WL_CONNECTED && idle_request_counter >= 15) {
      idle_request_counter = 0;
      HTTPClient http;
...
void loop() {
  ...
  esp_to_charge_module_communication();
  swser_communication();
