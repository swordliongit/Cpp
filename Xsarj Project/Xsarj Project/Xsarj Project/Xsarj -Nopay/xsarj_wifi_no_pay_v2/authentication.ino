...
String serverName_auth = "/web/session/authenticate";
...
void http_Post_Take_Session_Id() {
  ...
  if (currentMillis - previousMillis_post_req_auth >= interval_post_req_auth) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      http.begin(server_url+serverName_auth);
      ...
      int httpResponseCode = http.POST("{\"jsonrpc\":\"2.0\",\"params\":{\"login\":\""  + server_username + "\",\"password\":\"" + server_password + "\",\"db\":\"" + server_db_name  + "\"}}");
      ...
      Serial.print("Wifi Setup HTTP Post authentication Response code: ");
...
void http_Post_Take_Session_Id_Setup() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.print("Wifi HTTP Post authentication Response code: ");
    Serial.println(httpResponseCode);
