    ...
    if (relay_1_cloud_value == "0" && relay_1_cloud_value != relay_1_cloud_value_last) {
      ...
      if (charge_counter >= 0) {
        if ((sum_of_power / charge_counter) < 7.4) {
          charge_cost = String(((charge_counter / 60) * (-1 * package_price_1)));
        }
        if ((sum_of_power / charge_counter) >= 7.4 && (sum_of_power / charge_counter) < 11) {
          charge_cost = String(((charge_counter / 60) * (-1 * package_price_2)));
        }
        if ((sum_of_power / charge_counter) >= 11) {
          charge_cost = String(((charge_counter / 60) * (-1 * package_price_3)));
        }
...
void check_Money_Finish() {
  if (charge_counter > 0) {
    ...
    if (-1*charge_cost.toFloat() >= (partner_total_payment - package_price_3 - 2)) {
      relay_1_cloud_value = "0";
      control_Relays();
      error_code_6 = "1";
