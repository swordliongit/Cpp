...
void swSer_send_Data_Energy(String swSer_String) {
  swSer.println("ev_" + swSer_String + "!");
}
...
void swSer_get_data() {
  if (swSer.available() > 0) {
    second_esp_serial_data = swSer.readStringUntil('\n');
    Serial.println("Data From Second Esp: " + second_esp_serial_data);
    yield();
    if (second_esp_serial_data.indexOf("ocpp_") >= 0 && second_esp_serial_data.indexOf("!") > 0) {
      second_esp_serial_data = second_esp_serial_data.substring(second_esp_serial_data.indexOf("p_") + 2 , second_esp_serial_data.indexOf("!"));
    }
    if (second_esp_serial_data == "running") {
      device_status = "active";
      Serial.println("BeginTransaction From Second Serial");
    }
    if (second_esp_serial_data == "finished") {
      device_status = "passive";
      Serial.println("EndTransaction From Second Serial");
    }
  }
}
