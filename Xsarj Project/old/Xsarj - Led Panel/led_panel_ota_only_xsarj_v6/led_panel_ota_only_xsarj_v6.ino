...
void anim_Wifi_Offline(PatternAnimator &p10)
{
    int ROW_START = 1;
    int COL_START = 0;
    p10.draw_pattern_static(p10.wifi_offline, ROW_START, COL_START);
    int msdelay = 250;
    delay(msdelay);
}
...
{
    dmd.drawMarquee(bChars, 10, (32 * DISPLAYS_ACROSS) - 1, 4);
    long start = millis();
    long timer = start;
    boolean ret = false;
    while (!ret) {
...
void loop(void)
{
    ...
    //    anim_StationError(p10, 1);
    //  }
    //  if (data_from_serial2 != "99") {
    //    anim_StationWaiting_custom(p10, "EKIN TARIM");
    //    // anim_StationWaiting(p10, 1);
    //  }
