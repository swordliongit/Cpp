...
unsigned long previousMillis_screen_timer = 0;
const long interval_screen_timer = 3000;

int screen_started = 0;
...
void energy_Analyzer_Relay_1() {
  ...
  if (currentMillis - previousMillis_relay_1_timer >= interval_relay_1_timer) {
    ...
    if (relay_1_cloud_value == "0" && screen_started > 0) {
      unsigned long currentMillis = millis();
      if (currentMillis - previousMillis_screen_timer >= interval_screen_timer) {
        previousMillis_screen_timer = currentMillis;
    ...
    if (relay_1_cloud_value == "0" && relay_1_cloud_value != relay_1_cloud_value_last) {
      ...
      delay(1000);
      swSer_send_Data_Pstatus("1");
      Serial.println("---- Panel Data Finished ---------------------");
...
void charge_Time_plug_Out_Status() {
  if (relay_1_cloud_value == "1" && sensor_2_value < 1.5) {
    ...
    if (currentMillis - previousMillis_plug_timer >= interval_plug_timer) {
      ...
      if (no_charging_counter > 30 && no_charging_counter < 115) {
        swSer_send_Data_Pstatus("3");
        Serial.println("---- Panel Data Error ---------------------");
      }
